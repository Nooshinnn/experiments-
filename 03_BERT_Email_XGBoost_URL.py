# File: 03_BERT_Email_XGBoost_URL.py
import pandas as pd
import re
import torch
...
for fold, (train_idx, val_idx) in enumerate(kf.split(df)):
    ...
    # Training (Email Agent only - URL Agent is XGBoost)
    ...
    for epoch in range(20):
        ...
        for i in progress_bar:
            ...
            comm = email_feat  # placeholder for training
    ...
    # ====================== EVALUATION (XGBoost on lexical features) ======================
    email_model.eval()
    ...
    with torch.no_grad():
        for i in range(0, len(val_df), 32):
            ...
            # XGBoost uses ONLY lexical features
            url_feat_np = val_url_feat.iloc[i:i+32].values.astype(np.float32)

            dtrain = xgb.DMatrix(train_url_feat.values, label=train_df['label'].values)
            model = xgb.train({'max_depth': 6, 'objective': 'binary:logistic', 'eval_metric': 'auc'}, 
                              dtrain, num_boost_round=100)

            dval = xgb.DMatrix(url_feat_np)
...
print(f"\nFinal Average for BERT (Email) + XGBoost (URL):")
...
result_df.to_csv("03_BERT_Email_XGBoost_URL_results.csv", index=False)
print("\nResults saved to 03_BERT_Email_XGBoost_URL_results.csv")
