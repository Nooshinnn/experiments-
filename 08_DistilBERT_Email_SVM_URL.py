# File: 08_DistilBERT_Email_SVM_URL.py
import pandas as pd
import re
import torch
...
for fold, (train_idx, val_idx) in enumerate(kf.split(df)):
    ...
    # ====================== EVALUATION (SVM on lexical features only) ======================
    email_model.eval()
    ...
    with torch.no_grad():
        for i in range(0, len(val_df), 32):
            ...
            # SVM uses ONLY lexical features
...
print(f"\nFinal Average for DistilBERT (Email) + SVM (URL):")
...
result_df.to_csv("08_DistilBERT_Email_SVM_URL_results.csv", index=False)
print("\nResults saved to 08_DistilBERT_Email_SVM_URL_results.csv")
