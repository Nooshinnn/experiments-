...
optimizer_mp = optim.AdamW(list(email_model.parameters()) + list(url_model.parameters()) + list(mp_model.parameters()), lr=8e-6, weight_decay=0.08)
optimizer_sc = optim.AdamW(list(email_model.parameters()) + list(url_model.parameters()) + list(sc_model.parameters()), lr=8e-6, weight_decay=0.08)
...
b = np.sum((mp_preds == true) & (sc_preds != true))   # MP correct, SC wrong
c = np.sum((mp_preds != true) & (sc_preds == true))   # SC correct, MP wrong
...
print("="*70)
print(f"Message Passing (2 rounds) only correct : {b}")
