# File: Adversarial_BAE_Different_Percentages.py
import pandas as pd
import re
import torch
...
def bae_attack(text, change_percent=0.30):
    words = text.split()
    if len(words) < 5:
        return text
    ...
    # Simple saliency (change in prediction)
    saliency_scores = []
    original_prob = None
    ...
    with torch.no_grad():
        ...
        u_in = url_tokenizer(dummy_url, padding=True, truncation=True, max_length=128, return_tensors="pt").to(device)
...
for percent in percentages:
    ...
    test_adv['adv_email'] = [bae_attack(str(t), change_percent=percent) for t in tqdm(test_adv['email_text'], desc=f"BAE {percent*100:.0f}%")]
