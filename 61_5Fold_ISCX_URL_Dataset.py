# File: 61_5Fold_ISCX_URL_Dataset.py
import pandas as pd
import torch
...
print("="*160)
print("5-FOLD CV - ISCX-URL-2016 DATASET")
print("="*160)

# Model classes (copy from Script 59)
# ====================== MODEL ======================
...
# Load ISCX-URL-2016
df = pd.read_csv("URL-2016.csv")   # Change to your actual filename
df = df.rename(columns={"URL": "url", "Label": "label"})   # Adjust column names
df = df[df['label'].isin([0, 1])].reset_index(drop=True)

# Create synthetic email for full model
df['email_text'] = df['url'].apply(lambda u: f"Dear user, please click the link below to verify your account: {u}. Thank you.")

X = df.reset_index(drop=True)
y = df['label'].values

# Rest of the code is identical to Script 59
# 5-Fold
