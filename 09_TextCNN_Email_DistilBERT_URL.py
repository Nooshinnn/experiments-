# File: 09_TextCNN_Email_DistilBERT_URL.py
import pandas as pd
import re
import torch
...
for fold, (train_idx, val_idx) in enumerate(kf.split(df)):
    ...
    url_model = DistilBertWrapper().to(device)

    optimizer = optim.AdamW(list(email_model.parameters()) + list(url_model.parameters()), lr=8e-6, weight_decay=0.08)
    ...
    for epoch in range(20):
        ...
        if epoch == 2:
            print("    → No base layers to unfreeze (TextCNN)")
            # For TextCNN we don't have a frozen base like BERT
    ...
    with torch.no_grad():
        for i in range(0, len(val_df), 32):
            ...
            url_feat = url_model(url_inputs.input_ids, url_inputs.attention_mask)
            comm = torch.cat([email_feat, url_feat], dim=1)
            prob = comm.mean(dim=1).sigmoid().cpu().numpy()
...
print(f"\nFinal Average for TextCNN (Email) + DistilBERT (URL):")
...
result_df.to_csv("09_TextCNN_Email_DistilBERT_URL_results.csv", index=False)
print("\nResults saved to 09_TextCNN_Email_DistilBERT_URL_results.csv")
