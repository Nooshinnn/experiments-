...
# PPO Version
class PPODynamicWeight(nn.Module):
    def __init__(self, dim=128):
        super().__init__()
        self.fc = nn.Sequential(nn.Linear(dim*2, 64), nn.ReLU(), nn.Dropout(0.3), nn.Linear(64, 2))
    def forward(self, e, u):
        x = torch.cat([e, u], dim=1)
        return torch.softmax(self.fc(x), dim=1)

class PPOMP(nn.Module):
    def __init__(self, rounds=2):
        super().__init__()
        self.rounds = rounds
        self.update_e = nn.Sequential(nn.Linear(256, 128), nn.ReLU(), nn.Dropout(0.3))
        self.update_u = nn.Sequential(nn.Linear(256, 128), nn.ReLU(), nn.Dropout(0.3))
        self.ppo_weight = PPODynamicWeight()
        self.fc = nn.Linear(256, 1)
    def forward(self, e, u):
        for _ in range(self.rounds):
            e = self.update_e(torch.cat([e, u], dim=1))
            u = self.update_u(torch.cat([u, e], dim=1))
        weights = self.ppo_weight(e, u)
        fused = weights[:, 0:1] * e + weights[:, 1:2] * u
...
def train_and_evaluate(ModelClass, name):
    ...
    comm_model = ModelClass().to(device)
    ...
    for epoch in range(25):
        comm_model.train()
        for i in tqdm(range(0, len(train_df), 8), desc=f"Epoch {epoch+1}", leave=False):
...
print("="*70)
print(f"Original Message Passing    → F1: {original_f1:.4f} | Acc: {original_acc:.4f}")
print(f"PPO Dynamic Weighting       → F1: {ppo_f1:.4f} | Acc: {ppo_acc:.4f}")
print(f"Difference (PPO - Original) → F1: {ppo_f1 - original_f1:.4f}")
