# File: 01_BERT_Email_DistilBERT_URL.py
import pandas as pd
import re
import torch
...
print(f"\nFinal Average for BERT (Email) + DistilBERT (URL):")
...
print(f"  Log Loss: {avg_metrics.get('log_loss',0):.4f}")

# Save results
result_df = pd.DataFrame([avg_metrics])
result_df.to_csv("01_BERT_Email_DistilBERT_URL_results.csv", index=False)
print("\nResults saved to 01_BERT_Email_DistilBERT_URL_results.csv")
