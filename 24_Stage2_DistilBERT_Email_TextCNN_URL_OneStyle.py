# File: Stage2_DistilBERT_Email_TextCNN_URL_OneStyle.py
import pandas as pd
import re
import torch
...
url_tokenizer = AutoTokenizer.from_pretrained("distilbert-base-uncased")   # TextCNN uses same tokenizer
...
for fold, (train_idx, val_idx) in enumerate(kf.split(df)):
    ...
    for epoch in range(max_epochs):
        ...
        for i in progress_bar:
            ...
            u_feat = url_model(u_in.input_ids)
...
result_df.to_csv(f"Stage2_DistilBERT_Email_TextCNN_URL_{communication_name.replace(' ', '_')}_results.csv", index=False)
print(f"\nResults saved to Stage2_DistilBERT_Email_TextCNN_URL_{communication_name.replace(' ', '_')}_results.csv")
