# File: 06_DistilBERT_Email_RandomForest_URL.py
import pandas as pd
import re
import torch
...
for fold, (train_idx, val_idx) in enumerate(kf.split(df)):
    ...
    # ====================== EVALUATION (RandomForest on lexical features only) ======================
    email_model.eval()
...
print(f"\nFinal Average for DistilBERT (Email) + RandomForest (URL):")
...
result_df.to_csv("06_DistilBERT_Email_RandomForest_URL_results.csv", index=False)
print("\nResults saved to 06_DistilBERT_Email_RandomForest_URL_results.csv")
