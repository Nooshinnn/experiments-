# File: Original_MessagePassing_Training_With_EarlyStopping.py
import pandas as pd
import re
import torch
...
print("="*80)
print("Original Message Passing (2 rounds) Training + Validation Early Stopping")
print("="*80)
...
class OriginalMessagePassing(nn.Module):
...
comm_model = OriginalMessagePassing(rounds=2).to(device)
...
print("\nStarting Original Message Passing Training with Validation Early Stopping...\n")
...
for epoch in range(40):
    ...
    avg_val_loss = val_loss / (len(test_df) // batch_size + 1)

    print(f"Epoch {epoch+1} | Train Loss: {train_loss/len(train_df):.4f} | Val Loss: {avg_val_loss:.4f}")
...
print("\n" + "="*70)
print("ORIGINAL MESSAGE PASSING TRAINING FINISHED")
print("="*70)
print(f"Final Results → Accuracy: {acc:.4f} | F1: {f1:.4f} | MCC: {mcc:.4f}")
print("Best model saved as original_best_model.pth")
