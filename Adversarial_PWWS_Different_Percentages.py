# File: Adversarial_PWWS_Different_Percentages.py
import pandas as pd
import re
import torch
...
print("="*90)
print("ADVERSARIAL ATTACK: PWWS (Probability Weighted Word Saliency)")
...
def pwws_attack(text, change_percent=0.30):
    words = text.split()
    if len(words) < 5:
        return text
    ...
    # Compute saliency for each word
    saliency = []
    original_prob = None
    
    with torch.no_grad():
        e_in = email_tokenizer([text], padding=True, truncation=True, max_length=256, return_tensors="pt").to(device)
        u_in = url_tokenizer([""], padding=True, truncation=True, max_length=128, return_tensors="pt").to(device)  # dummy URL
    ...
    for i, word in enumerate(words):
        ...
        with torch.no_grad():
            e_in = email_tokenizer([masked_text], padding=True, truncation=True, max_length=256, return_tensors="pt").to(device)
            e_feat = email_model(e_in.input_ids, e_in.attention_mask)
            u_feat = url_model(u_in.input_ids, u_in.attention_mask)  # dummy
            logits = comm_model(e_feat, u_feat)
            new_prob = torch.sigmoid(logits).item()
        
        saliency.append(abs(original_prob - new_prob))  # higher = more important
    
    # Attack most salient words first
    indices = np.argsort(saliency)[::-1][:num_changes]
    ...
    for idx in indices:
        ...
        top_tokens = logits[0, mask_idx].topk(30).indices
...
# ====================== RUN PWWS WITH DIFFERENT PERCENTAGES ======================
percentages = [0.10, 0.20, 0.30, 0.40, 0.50]

print("\nRunning PWWS attacks...\n")
clean = evaluate(use_adv=False)
print(f"Clean Baseline → F1: {clean['f1']:.4f} | Acc: {clean['accuracy']:.4f}")

for percent in percentages:
    print(f"\n→ Attacking with {percent*100:.0f}% changes (PWWS)...")
    test_adv = test_df.copy()
    test_adv['adv_email'] = [pwws_attack(str(t), change_percent=percent) for t in tqdm(test_adv['email_text'])]
    
    adv = evaluate(use_adv=True)
    print(f"   F1: {adv['f1']:.4f} | Drop: {clean['f1'] - adv['f1']:.4f}")

print("\nPWWS attack experiment completed.")
