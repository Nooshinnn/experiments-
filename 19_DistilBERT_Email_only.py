# File: 19_DistilBERT_Email_only.py
import pandas as pd
import re
import torch
...
for fold, (train_idx, val_idx) in enumerate(kf.split(df)):
    ...
    for epoch in range(20):   # Old max epochs = 20
        email_model.train()
        ...
        # Old simple early stopping (if loss > 0.9 after epoch 8)
...
print(f"\nFinal Average for DistilBERT (Email-only):")
for k, v in avg_metrics.items():
    print(f"  {k.replace('_', ' ').title()}: {v:.4f}")

result_df = pd.DataFrame([avg_metrics])
result_df.to_csv("19_DistilBERT_Email_only_results.csv", index=False)
print("\nResults saved to 19_DistilBERT_Email_only_results.csv")
